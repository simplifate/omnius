...
namespace FSS.Omnius.Controllers.EPK
{
    ...
    public class OrdersController : Controller
    {
        ...
        public ActionResult Create()
        {
            ...
            E.DBItem user = e.GetDynamicTable("Users").Select().where(c => c.column("ad_id").Equal(HttpContext.GetLoggedUser().UserName)).ToList().FirstOrDefault();
            ViewData["uic451"] = HttpContext.GetLoggedUser().DisplayName;
        ...
        public ActionResult Create(FormCollection fc)
        {
            if (fc.AllKeys.Contains("uic470"))
            {
                ...
                try
                {
                    ...
                    item.createProperty(-4, "purchase_year", DateTime.Now.Year);
                    ...
                    item.createProperty(-8, "client_name", user != null ? user["sapid1"] : "");
                    item.createProperty(-9, "client_sapid2", user != null ? user["sapid2"] : "");
                    item.createProperty(-11, "client_function", "");
                    item.createProperty(-12, "client_cost_centre", "");
                    item.createProperty(-13, "client_company_name", "");
                    item.createProperty(-14, "approver_name", "");
                    item.createProperty(-15, "approver_sapid2", "");
                    item.createProperty(-16, "approver_ouid", "");
                    item.createProperty(-17, "approver_date", DateTime.Now);
