        ...
        protected override void BaseExecution(MarshalByRefObject transaction)
        {
            ...
            if (isClusterCreated != true)
            {
                sqlString = string.Format(
                    ...
                    "SET @sql= CONCAT('ALTER TABLE ', @realTableName, ' ADD CONSTRAINT PK_{3}_', @realTableName, ' PRIMARY KEY NONCLUSTERED (', @{2}, ');' , " +
                    "' CREATE CLUSTERED INDEX index_', @{0}, @{1} , ' ON ', @realTableName, '(', @{2},');');" +
                    "exec (@sql)",
                    parAppName, parTableName, parColumns,application.Name);
            }
            else
            {
                sqlString = string.Format(
