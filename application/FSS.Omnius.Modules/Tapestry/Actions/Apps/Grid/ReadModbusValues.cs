...
{
    public class ReadModbusValues : Action
    {
        ...
        {
            get {
                return new string[] { "s$IP", "i$Port", "s$Unit", "s$StartAddress", "s$Length" };
            }
        }
        public override string[] OutputVar
        {
            get {
                return new string[] { "Result", "IsError" };
            }
        }
        public override string Name
        {
            get {
                return "Read modbus values";
            }
        }

        private static List<int> lastData = new List<int>();
        private JArray zeroArray;

        public override void InnerRun(Dictionary<string, object> vars, Dictionary<string, object> outputVars, Dictionary<string, object> InvertedInputVars, Message message)
        {
            if(zeroArray == null)
            {
                zeroArray = new JArray();
                for (int i = 0; i < 68; i++)
                {
                    zeroArray.Add(0);
                }
            ...
            try
            {
                string ip = (string)vars["IP"];

                if(string.IsNullOrEmpty(ip))
                {
                    outputVars["Result"] = zeroArray;
                    outputVars["IsError"] = false;
                    return;
                }

                byte unit = Convert.ToByte((string)vars["Unit"]);
                byte length = Convert.ToByte((string)vars["Length"]);
                ushort port = Convert.ToUInt16(vars["Port"]);
                ushort startAddress = Convert.ToUInt16(vars["StartAddress"]);

                byte[] values = new byte[] { };

                ModbusTCPMaster master = new ModbusTCPMaster(ip, (ushort)port);
                master.OnException += new ModbusTCPMaster.ExceptionData(Master_OnException);

                master.ReadHoldingRegister(3, unit, startAddress, length, ref values);

                int[] bytes = new int[values.Length];
                for (int x = 0; x < values.Length; x++)
                {
                    bytes[x] = Convert.ToInt32(values[x]);
                }

                outputVars["IsError"] = false;
                outputVars["Result"] = JArray.FromObject(bytes);
                lastData = bytes.ToList();
            ...
            catch (Exception e)
            {
                ...
                outputVars["IsError"] = lastData.Count > 0 ? false : true;
                outputVars["Result"] = lastData.Count > 0 ? JArray.FromObject(lastData) : zeroArray;
            }
        }

        private void Master_OnException(ushort id, byte unit, byte function, byte exception)
        {
            string exc = "Modbus says error: ";
            switch (exception) {
                case ModbusTCPMaster.excIllegalFunction: exc += "Illegal function!"; break;
                case ModbusTCPMaster.excIllegalDataAdr: exc += "Illegal data adress!"; break;
                case ModbusTCPMaster.excIllegalDataVal: exc += "Illegal data value!"; break;
                case ModbusTCPMaster.excSlaveDeviceFailure: exc += "Slave device failure!"; break;
                case ModbusTCPMaster.excAck: exc += "Acknoledge!"; break;
                case ModbusTCPMaster.excGatePathUnavailable: exc += "Gateway path unavailbale!"; break;
                case ModbusTCPMaster.excExceptionTimeout: exc += "Slave timed out!"; break;
                case ModbusTCPMaster.excExceptionConnectionLost: exc += "Connection is lost!"; break;
                case ModbusTCPMaster.excExceptionNotConnected: exc += "Not connected!"; break;
            }
