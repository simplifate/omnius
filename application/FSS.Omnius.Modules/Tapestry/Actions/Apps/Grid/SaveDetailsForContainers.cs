...
{
    class SaveDetailsForContainersAction : Action
    {
        ...
        public override string Name
        {
            get
            {
                return "Grid:Save Details For Containers";
            }
        }
        ...
        public override void InnerRun(Dictionary<string, object> vars, Dictionary<string, object> outputVars, Dictionary<string, object> InvertedInputVars, Message message)
        {
            ...
            double totalActiveMinerCount = Convert.ToDouble(miners.Where(m => m["status"].ToString() == "up" || m["status"].ToString() == "warning").ToList().Count);
            ...
            foreach (DBItem combPlacement in tRigPlacement.Select("Profile_id", "Container_id").Group(ESqlFunction.none, null, "Profile_id", "Container_id").ToList())
            {
                ...
                // get miners
                IEnumerable<JToken> combMiners = miners.Where(m => combRigNames.Contains(m["name"].ToString()));
                IEnumerable<JToken> combTestingMiners = miners.Where(m => combRigNames_Testing.Contains(m["name"].ToString()));
            ...
            foreach (DBItem container in tContainers.Select().ToList())
            {
                ...
                var allContainerMiners = miners.Where(m => containerRigNames.Contains(m["name"].ToString()));
                var allContainerTestingMiners = miners.Where(m => containerRigNames_Testing.Contains(m["name"].ToString()));
