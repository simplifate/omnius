...
namespace FSS.Omnius.Modules.Tapestry2.Actions.App
{
    public partial class Grid : ActionManager
    {
        ...
        [Action(30129, "Read modbus values", "Result", "IsError")]
        public static (JArray, bool) ReadModbusValues(COREobject core, string IP, byte Unit, byte Length, UInt16 Port, UInt16 StartAddress)
        {
            JArray zeroArray = new JArray();
            for (int i = 0; i < 68; i++)
            {
                zeroArray.Add(0);
            }
            ...
            try
            {
                ...
                return (JArray.FromObject(bytes), false);
            }
            catch (Exception e)
            {
                Watchtower.OmniusInfo.Log(e.Message);
                return (lastData.Count > 0 ? JArray.FromObject(lastData) : zeroArray, lastData.Count > 0);
            }
        }

        private static void Master_OnException(ushort id, byte unit, byte function, byte exception)
        {
