...
namespace FSS.Omnius.Modules.Tapestry2.Actions.App
{
    ...
    {
        [Action(3005, "Get wallets income Action")]
        public static void GetIncomes(COREobject core, string HistoryTableName = "TransactionHistory", string WalletsTableName = "Wallets")
        {
            var action = new GetIncomesAction();
            action.Run(core, HistoryTableName, WalletsTableName);
        }
        ...
        class GetIncomesAction
        {
            ...
            public void Run(COREobject core, string historyTableName, string walletsTableName)
            {
                _core = core;
                _db = _core.Entitron;
                ...
                try
                {
                    lock (threadLock)
                    {
                        ...
                        foreach (DBItem item in historyTable.Select("Transaction_id", "Currency_code", "Wallet_Address").ToList())
                        {
                            ...
                            {
                                Watchtower.OmniusLog.Log($"{e.Message}", Watchtower.OmniusLogLevel.Error);
                            }
                        }
                        if (isCurrencyNotFound)
                        {
                            Watchtower.OmniusLog.Log($"Transaction history contains data of non-existing wallet(s)! {String.Join(", ", currenciesNotFound.Select(t => string.Format("[{0} {1}]", t.Item2, t.Item1)))}", Watchtower.OmniusLogLevel.Warning);
                        }
            ...
            private void LoadXmrTx()
            {
                ...
                try
                {
                    ...
                    using (StreamReader reader = new StreamReader(stream2, Encoding.UTF8))
                    {
                        ...
                        foreach (var transactionIn in jtokenResult["in"])
                        { //iterate each object in the array of IN
                          //if (x["in"]["amount"] > 0)
                          //{
