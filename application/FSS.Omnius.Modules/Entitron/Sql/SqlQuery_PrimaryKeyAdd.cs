...
namespace FSS.Omnius.Modules.Entitron.Sql
{
    class SqlQuery_PrimaryKeyAdd : SqlQuery_withAppTable
    {
        ...
        protected override void BaseExecution(MarshalByRefObject transaction)
        {
            ...
            {
                sqlString =
                    $"ALTER TABLE [{realTableName}] ALTER COLUMN {keyColumns} INT IDENTITY; ALTER TABLE [{realTableName}] ADD CONSTRAINT PK_{realTableName} PRIMARY KEY NONCLUSTERED ({keyColumns});" +
                    $"CREATE CLUSTERED INDEX index_{application.Name}{table.tableName} ON [{realTableName}]({keyColumns});";
            ...
            {
                sqlString =
                    $"ALTER TABLE [{realTableName}] ADD CONSTRAINT PK_{realTableName} PRIMARY KEY NONCLUSTERED({keyColumns});";
