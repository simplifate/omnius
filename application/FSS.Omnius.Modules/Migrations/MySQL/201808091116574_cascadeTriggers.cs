namespace FSS.Omnius.Modules.Migrations.MySQL
{
    ...
    public partial class cascadeTriggers : DbMigration
    {
        ...
        {
            Sql("CREATE TRIGGER `TRG_C:Entitron_DbColumn` BEFORE DELETE ON `Entitron_DbColumn` FOR EACH ROW BEGIN DELETE FROM `Entitron_DbRelation` WHERE `LeftColumnId` = OLD.Id OR `RightColumnId` = OLD.Id; END");
            ...
            Sql("CREATE TRIGGER `TRG_C:Entitron_DbTable` BEFORE DELETE ON `Entitron_DbTable` FOR EACH ROW BEGIN DELETE FROM `Entitron_DbRelation` WHERE `LeftTableId` = OLD.Id OR `RightTableId` = OLD.Id; DELETE FROM `Entitron_DbColumn` WHERE `DbTableId` = OLD.Id; DELETE FROM `Entitron_DbIndex` WHERE `DbTableId` = OLD.Id; END");
            ...
            Sql("CREATE TRIGGER `TRG_C:MozaicB_Page` BEFORE DELETE ON `MozaicB_Page` FOR EACH ROW BEGIN DELETE FROM `MozaicB_Components` WHERE `MozaicBootstrapPageId` = OLD.Id; DELETE FROM `Mozaic_Js` WHERE `MozaicBootstrapPageId` = OLD.Id; UPDATE `TapestryDesigner_ResourceItems` SET `BootstrapPageId` = NULL WHERE `BootstrapPageId` = OLD.Id; UPDATE `Tapestry_Blocks` SET `BootstrapPageId` = NULL WHERE `BootstrapPageId` = OLD.Id; END");
