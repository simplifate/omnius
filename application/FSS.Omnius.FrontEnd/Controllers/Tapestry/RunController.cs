...
namespace FSS.Omnius.Controllers.Tapestry
{
    ...
    public class RunController : Controller
    {
        ...
        public ActionResult Index(string appName, string locale = "", string blockIdentify = null, int modelId = -1, string message = null, string messageType = null, string registry = null)
        {
            ...
            if (!string.IsNullOrEmpty(block.RoleWhitelist) && !block.RoleWhitelist.Split(',').Any(r => core.User.HasRole(r, core.Application.Id)))
            ...
            T t = new T(core.User.Locale);
            var result = new Modules.Tapestry.Tapestry(core).innerRun(block, "INIT", modelId, fc, -1, blockDependencies);
            ...
            foreach (var resourceMappingPair in block.ResourceMappingPairs.Where(r => r.SourceTableName != null && String.IsNullOrEmpty(r.SourceColumnName)).ToList())
            {
                ...
                if (resourceMappingPair.relationType.StartsWith("V:"))
                {
                    ...
                    {
                        if (noConditions || ConditionalFilteringService.MatchConditionSets(conditionSets, entitronRow, tapestryVars))
                        {
        ...
        public string PrepareAlert(Message message)
        {
        ...
        {
            return new T(COREobject.i.User.Locale);
        }
        ...
        public ActionResult Index(string appName, string button, FormCollection fc, string blockIdentify = null, int modelId = -1, int deleteId = -1)
        {
            ...
            // run
            var result = new Modules.Tapestry.Tapestry(core).run(block, button, modelId, fc, deleteId);
            Session["CrossBlockRegistry"] = core.CrossBlockRegistry;
    ...
    public class UnauthorizedRunController : Controller
    {
        ...
        public ActionResult Get(string appName, string button, string blockIdentify = null, int modelId = -1)
        {
            try
            {
                ...
                // get block
                Block block = getBlockWithWF(core, appName, blockIdentify);
                if (block == null)
                    return new HttpStatusCodeResult(404);

                blockDependencies.Add("Translator", GetTranslator());
                // run
                var result = new Modules.Tapestry.Tapestry(core).run(block, button ?? "INIT", modelId, new FormCollection(), -1, blockDependencies);
                return RedirectToRoute("Run", new { appName = appName, blockIdentify = result.Item2.Name, modelId = modelId, message = result.Item1.ToUser(), messageType = result.Item1.Type.ToString(), registry = JsonConvert.SerializeObject(result.Item3) });
            }
            catch (Exception e)
            {
        ...
        public ActionResult GetJson(string appName, string button, string blockIdentify = null, int modelId = -1)
        {
            ...
            blockDependencies.Add("Translator", this.GetTranslator());
            // run
            var result = new Modules.Tapestry.Tapestry(core).jsonRun(block, button, modelId, new FormCollection());
        ...
        private bool Authorize()
        {
            ...
            {
                COREobject core = COREobject.i;
                core.User = core.Context.Users.SingleOrDefault(u => u.UserName == userName);
        ...
        private Block getBlockWithWF(COREobject core, string appName, string blockName)
        {
            return blockName != null
                ? core.Context.Blocks
                    .Include(b => b.SourceTo_ActionRules)
                    .FirstOrDefault(b => b.WorkFlow.ApplicationId == core.Application.Id && b.Name.ToLower() == blockName.ToLower())
                : core.Context.Blocks.Include(b => b.ResourceMappingPairs).Include(b => b.SourceTo_ActionRules).FirstOrDefault(b => b.WorkFlow.ApplicationId == core.Application.Id && b.WorkFlow.InitBlockId == b.Id);
        }
